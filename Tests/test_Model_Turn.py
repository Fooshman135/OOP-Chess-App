...
class TestIsPlayerAttemptingToCastle:
	...
	def test_works_for_expected_input(self, test_input, expected):
		...
		result = test_input.is_player_attempting_to_castle()
		assert result == expected, "Expected Turn to identify player's intent to castle."	
...
class TestIsLegalMoveCastling:
	...
	@pytest.mark.parametrize("test_input, expected",[
		(turn_one, True),			# No complications
		(turn_two, False),			# King was moved in a previous turn
		(turn_three, False),		# Rook was moved in a previous turn
		(turn_four, False),			# King passes through check while castling
		(turn_five, False),			# King's path is blocked by enemy piece
		(turn_six, False),			# King's path is blocked by own piece
		(turn_seven, False),		# King starts off in check
		(turn_eight, False),		# Rook's path is blocked by enemy piece
	])
	def test_works_for_expected_input(self, test_input, expected):
		# Returns expected value when passed valid input.
