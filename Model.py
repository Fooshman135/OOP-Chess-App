    ...
    def game_turn_loop(self):
        ...
        while True:
            ...
            while True:
                ...
                # Determine if player is trying to castle (regardless of whether its a legal move).
                if current_turn.is_player_attempting_to_castle() is True:
                    
                    # Player is trying to castle.

                    # Determine if move is a legal castling move.
                    if current_turn.is_legal_move_castling() is True:
                        # Player successfully tried to castle. 
                        # No need to check validation types 4 and 5.
                        current_turn.is_castling = True

                    else:
                        # Player unsuccessfully tried to castle.
                        # Reject move.
                        display_error_message(8)
                        del current_turn
                        continue

                else:
                    # Player is not trying to castle.
...
class Turn(object):
    ...
    def is_player_attempting_to_castle(self):
        # Returns True if the player is trying to castle, False otherwise.

        # Need to confirm each of the following:
        # (1) Starting square contains a King (don't need to check color because that was done previously in INPUT VALIDATION TYPE 2).
        # (2) Ending square is in the same row as starting square (this is first half of INPUT VALIDATION TYPE 4).
        # (3) Ending square is two squares away from starting square (this is second half of INPUT VALIDATION TYPE 4).
        ...
        # If you have reached this point, the player is attempting to cancel.
        return True
    ...
    def is_legal_move_castling(self):
        ...
        # Need to confirm each of the following:
        # (4) King hasn't moved previously.
